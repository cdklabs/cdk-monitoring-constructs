...
import {AddAlarmProps, AlarmNamingStrategy, IAlarmDedupeStringProcessor} from "../../../lib";
...
test("no dedupe: getName returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getName(DummyTicketingAlarmProps)).toBe("first-second-CustomAlarmSuffix");
});

test("no dedupe: getDedupeString returns undefined", () => {
    expect(new AlarmNamingStrategy("first", "second").getDedupeString(DummyTicketingAlarmProps)).toBeUndefined();
});

test("no dedupe: getWidgetLabel returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getWidgetLabel(DummyTicketingAlarmProps)).toBe("second CustomAlarmSuffix");
});

test("dedupe: getName returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getName(DummyTicketingAlarmPropsWithDedupe)).toBe("first-second-CustomAlarmSuffix");
});

test("dedupe: getDedupeString returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getDedupeString(DummyTicketingAlarmPropsWithDedupe)).toBe(
        "first-second-TestingDedupe",
    );
});

test("dedupe: getWidgetLabel returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getWidgetLabel(DummyTicketingAlarmPropsWithDedupe)).toBe("second CustomAlarmSuffix");
});

test("dedupe: getDedupeString returns expected value with override", () => {
    expect(new AlarmNamingStrategy("first", "second").getDedupeString(DummyTicketingAlarmPropsWithDedupeOverride)).toBe("DedupeOverride");
});

test("dedupe: getDedupeString returns expected value with override and disambiguator", () => {
    expect(new AlarmNamingStrategy("first", "second").getDedupeString(DummyTicketingAlarmPropsWithDedupeOverrideAndDisambiguator)).toBe(
        "DedupeOverride",
    );
});

test("dedupe and disambiguator: getName returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getName(DummyTicketingAlarmPropsWithDedupeAndDisambiguator)).toBe(
        "first-second-CustomAlarmSuffix-TestingDisambiguator",
    );
});

test("alarm name override: getName returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getName(DummyTicketingAlarmPropsWithNameOverride)).toBe("AlarmNameOverride");
});

test("dedupe and disambiguator: getDedupeString returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getDedupeString(DummyTicketingAlarmPropsWithDedupeAndDisambiguator)).toBe(
        "first-second-TestingDedupe",
    );
});

test("dedupe and disambiguator: getWidgetLabel returns expected value", () => {
    expect(new AlarmNamingStrategy("first", "second").getWidgetLabel(DummyTicketingAlarmPropsWithDedupeAndDisambiguator)).toBe(
        "second CustomAlarmSuffix TestingDisambiguator",
    );
});
...
test("dedupe string gets processed: override", () => {
    ...
    expect(
        new AlarmNamingStrategy("first", "second", new CustomStrategy()).getDedupeString({
