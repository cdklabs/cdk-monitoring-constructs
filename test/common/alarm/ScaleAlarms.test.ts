...
function verify(
  ...
  const requestedDisambiguators = new Set(
    facade.createdAlarms().map((a) => a.disambiguator),
  );
  expect(requestedDisambiguators).toEqual(new Set(["Critical", "Warning"]));
  ...
  expect(Object.keys(templateAllAlarms).length).toBe(
    expectedNumberOfCriticalAlarms * 2 + expectedNumberOfWarningAlarms,
  );
  ...
  const templateRollbackAlarms = template.findResources(
    "AWS::CloudWatch::Alarm",
    Match.objectLike({
      Properties: {
        AlarmName: Match.stringLikeRegexp("-Rollback$"),
        AlarmActions: Match.absent(),
        ActionsEnabled: Match.anyValue(),
        AlarmDescription: Match.anyValue(),
        ComparisonOperator: Match.anyValue(),
        DatapointsToAlarm: Match.anyValue(),
        EvaluationPeriods: Match.anyValue(),
  ...
  expect(Object.keys(templateRollbackAlarms).length).toBe(
    expectedNumberOfCriticalAlarms,
  );
  ...
  Object.values(templateCriticalAlarms).forEach((requestedAlarmCfn) => {
    ...
    const clonedAlarmCfn = Object.values(templateRollbackAlarms).find((a) => {
