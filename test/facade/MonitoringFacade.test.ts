...
describe("test of defaults", () => {
  ...
  test("Given clone function, creates cloned monitors and returns them.", () => {
    ...
    expect(clones.map((a) => a.action)).toEqual(
      expect.arrayContaining(
        Array(expectedNumberOfClonedAlarms).fill(noopAction()),
      ),
    );
    expect(clones.map((a) => a.disambiguator)).toEqual(
      expect.arrayContaining(
        Array(expectedNumberOfClonedAlarms).fill("UpperBoundCritical"),
      ),
    );
