...
import {AlarmWithAnnotation, OpenSearchClusterMonitoring, OpenSearchClusterStatus} from "../../../lib";
...
[
    ...
    test(`snapshot test: no alarms for ${domain.node.id}`, () => {
        const scope = new TestMonitoringScope(stack, "Scope-NoAlarms-" + domain.node.id);
    ...
    test(`snapshot test: all alarms for ${domain.node.id}`, () => {
        const scope = new TestMonitoringScope(stack, "Scope-AllAlarms-" + domain.node.id);
