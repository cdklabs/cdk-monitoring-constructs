...
import {Cluster, Ec2Service, Ec2TaskDefinition, EcrImage, FargateService, FargateTaskDefinition} from "aws-cdk-lib/aws-ecs";
import {ApplicationLoadBalancer, ApplicationTargetGroup, IApplicationLoadBalancerTarget} from "aws-cdk-lib/aws-elasticloadbalancingv2";

import {AlarmWithAnnotation, Ec2ServiceMonitoring, FargateServiceMonitoring} from "../../../lib";
...
function createAlb(stack: Stack, vpc: IVpc, target: IApplicationLoadBalancerTarget) {
...
function importAlb(stack: Stack) {
    ...
    const tgArn = "arn:aws:elasticloadbalancing:us-west-2:123456789012:targetgroup/my-target-group/50dc6c495c0c9188";
    const applicationLoadBalancer = ApplicationLoadBalancer.fromApplicationLoadBalancerAttributes(stack, "ImportedALB", {
        loadBalancerArn: albArn,
        securityGroupId: "sg-abcdefg",
    });
    const applicationTargetGroup = ApplicationTargetGroup.fromTargetGroupAttributes(stack, "ImportedApplicationTargetGroup", {
...
test.each([createAlb, importAlb])("snapshot alb test: no alarms - %#", (lbResourcesFactory) => {
    ...
    taskDefinition.addContainer("Container", {image}).addPortMappings({containerPort: 8080});
...
test.each([createAlb, importAlb])("snapshot alb test: all alarms - %#", (lbResourcesFactory) => {
    const stack = new Stack();
...
test.each([createAlb, importAlb])("snapshot ec2 alb test: no alarms - %#", (lbResourcesFactory) => {
    const stack = new Stack();
    ...
    taskDefinition.addContainer("Container", {image, memoryReservationMiB: 128}).addPortMappings({containerPort: 8080});
...
test.each([createAlb, importAlb])("snapshot ec2 alb test: all alarms - %s", (lbResourcesFactory) => {
    const stack = new Stack();
