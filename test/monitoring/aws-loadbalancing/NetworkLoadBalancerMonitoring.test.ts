...
import {AlarmWithAnnotation, NetworkLoadBalancerMonitoring} from "../../../lib";
...
function createNlb() {
    ...
    const networkTargetGroup = networkLoadBalancer.addListener("Listener", {port: 80}).addTargets("Target", {port: 80});
    return {networkLoadBalancer, networkTargetGroup, stack};
}

function importNlb() {
    const stack = new Stack();
    const nlbArn = "arn:aws:elasticloadbalancing:us-west-2:123456789012:loadbalancer/network/my-load-balancer/50dc6c495c0c9188";
    const tgArn = "arn:aws:elasticloadbalancing:us-west-2:123456789012:targetgroup/my-target-group/50dc6c495c0c9188";
    const networkLoadBalancer = NetworkLoadBalancer.fromNetworkLoadBalancerAttributes(stack, "ImportedNLB", {
        loadBalancerArn: nlbArn,
    });
    const networkTargetGroup = NetworkTargetGroup.fromTargetGroupAttributes(stack, "ImportedNetworkTargetGroup", {
...
test.each([createNlb, importNlb])("snapshot nlb test: no alarms - %#", (factory) => {
    let props = factory();
...
test.each([createNlb, importNlb])("snapshot nlb test: all alarms - %#", (lbResourcesFactory) => {
