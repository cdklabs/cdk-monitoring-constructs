...
export class DynamicDashboardFactory extends Construct implements IDynamicDashboardFactory {
    ...
    constructor(scope: Construct, id: string, props: MonitoringDynamicDashboardsProps) {
        ...
        props.dashboardConfigs.forEach((dashboardConfig) => {
            ...
            if (Object.values<string>(DefaultDashboards).includes(dashboardConfig.name)) {
                throw new Error(`${dashboardConfig.name} is a reserved name and cannot be used`);
            }

            const renderingPreference = dashboardConfig.renderingPreference ?? DashboardRenderingPreference.INTERACTIVE_ONLY;
            const start: string = "-" + (dashboardConfig.range ?? Duration.hours(8).toIsoString());
            ...
            const dashboard = this.createDashboard(renderingPreference, dashboardConfig.name, {
                ...
                start,
                periodOverride: dashboardConfig.periodOverride ?? PeriodOverride.INHERIT,
            });
