...
export class DefaultDashboardFactory extends Construct implements IDashboardFactory, IDynamicDashboardFactory {
    ...
    constructor(scope: Construct, id: string, props: MonitoringDashboardsProps) {
        ...
        const renderingPreference = props.renderingPreference ?? DashboardRenderingPreference.INTERACTIVE_ONLY;
        const detailStart: string = "-" + (props.detailDashboardRange ?? Duration.hours(8)).toIsoString();
        const summaryStart: string = "-" + (props.summaryDashboardRange ?? Duration.days(14)).toIsoString();
        ...
        if (createDashboard) {
            ...
            this.dashboard = this.createDashboard(renderingPreference, "Dashboard", {
                ...
                start: detailStart,
                periodOverride: props.detailDashboardPeriodOverride ?? PeriodOverride.INHERIT,
            });
        ...
        if (createSummaryDashboard) {
            ...
            this.summaryDashboard = this.createDashboard(renderingPreference, "SummaryDashboard", {
                ...
                start: summaryStart,
                periodOverride: props.summaryDashboardPeriodOverride ?? PeriodOverride.INHERIT,
            });
    ...
    addSegment(props: IDashboardFactoryProps) {
        ...
        if ((props.overrideProps?.addToSummaryDashboard ?? true) && this.summaryDashboard) {
            this.summaryDashboard.addWidgets(...props.segment.summaryWidgets());
        }
        if ((props.overrideProps?.addToAlarmDashboard ?? true) && this.alarmDashboard) {
    ...
    protected createDashboard(renderingPreference: DashboardRenderingPreference, id: string, props: DashboardProps) {
