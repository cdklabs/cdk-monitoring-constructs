...
export class ElastiCacheClusterMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: ElastiCacheClusterMonitoringProps) {
        ...
        const clusterType = capitalizeFirstLetterOnly(ElastiCacheClusterType[props.clusterType]);
        ...
        if (props.clusterId) {
            this.clusterUrl = scope.createAwsConsoleUrlFactory().getElastiCacheClusterUrl(props.clusterId, props.clusterType);
        }

        const metricFactory = new ElastiCacheClusterMetricFactory(scope.createMetricFactory(), props);
        ...
        this.freeableMemoryMetric = metricFactory.metricAverageFreeableMemoryInBytes();
        ...
        this.itemsMemoryMetric = metricFactory.metricAverageCachedItemsSizeInBytes();
        ...
        for (const disambiguator in props.addMaxEvictedItemsCountAlarm) {
            ...
            const createdAlarm = this.elastiCacheAlarmFactory.addMaxEvictedItemsCountAlarm(
        ...
        for (const disambiguator in props.addMinFreeableMemoryAlarm) {
            ...
            const createdAlarm = this.elastiCacheAlarmFactory.addMinFreeableMemoryAlarm(
    ...
    createMemoryUsageWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            left: [this.freeableMemoryMetric, this.unusedMemoryMetric, this.swapMemoryMetric],
