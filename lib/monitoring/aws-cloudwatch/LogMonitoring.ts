import {GraphWidget, HorizontalAnnotation, IWidget, LogQueryWidget} from "aws-cdk-lib/aws-cloudwatch";

import {CloudWatchLogsMetricFactory, CloudWatchLogsMetricFactoryProps} from "./CloudWatchLogsMetricFactory";
import {
    AlarmFactory,
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class LogMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: LogMonitoringProps) {
        ...
        this.logGroupUrl = scope.createAwsConsoleUrlFactory().getCloudWatchLogGroupUrl(props.logGroupName);
        ...
        const metricFactory = new CloudWatchLogsMetricFactory(scope.createMetricFactory(), props);
    ...
    summaryWidgets(): IWidget[] {
        return [this.createTitleWidget(), this.createIncomingLogsWidget(FullWidth, DefaultSummaryWidgetHeight)];
    }
    ...
    widgets(): IWidget[] {
        if (this.pattern) {
            ...
            return [this.createTitleWidget(), this.createIncomingLogsWidget(FullWidth, DefaultGraphWidgetHeight)];
        }
    }
