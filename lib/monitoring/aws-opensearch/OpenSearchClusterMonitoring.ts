...
import {OpenSearchClusterMetricFactory, OpenSearchClusterMetricFactoryProps} from "./OpenSearchClusterMetricFactory";
import {
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class OpenSearchClusterMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: OpenSearchClusterMonitoringProps) {
        ...
        this.url = props.domain.domainName
            ? scope.createAwsConsoleUrlFactory().getOpenSearchClusterUrl(props.domain.domainName)
            : undefined;
        ...
        this.indexingLatencyAlarmFactory = new LatencyAlarmFactory(this.createAlarmFactory(indexingAlarmPrefix));
        this.indexingLatencyAnnotations = [];
        this.searchLatencyAlarmFactory = new LatencyAlarmFactory(this.createAlarmFactory(searchAlarmPrefix));
        this.searchLatencyAnnotations = [];
        this.clusterAlarmFactory = new OpenSearchClusterAlarmFactory(this.createAlarmFactory(clusterAlarmPrefix));
        this.clusterAnnotations = [];
        this.nodeAnnotations = [];
        this.usageAlarmFactory = new UsageAlarmFactory(this.createAlarmFactory(alarmPrefix));
        this.usageAnnotations = [];
        this.masterUsageAnnotations = [];

        const metricFactory = new OpenSearchClusterMetricFactory(scope.createMetricFactory(), props);
        this.tpsMetric = metricFactory.metricTps();
        this.p50IndexingLatencyMetric = metricFactory.metricIndexingLatencyP50InMillis();
        this.p90IndexingLatencyMetric = metricFactory.metricIndexingLatencyP90InMillis();
        this.p99IndexingLatencyMetric = metricFactory.metricIndexingLatencyP99InMillis();
        this.p50SearchLatencyMetric = metricFactory.metricSearchLatencyP50InMillis();
        this.p90SearchLatencyMetric = metricFactory.metricSearchLatencyP90InMillis();
        this.p99SearchLatencyMetric = metricFactory.metricSearchLatencyP99InMillis();
        ...
        this.masterJvmMemoryPressureMetric = metricFactory.metricMasterJvmMemoryPressure();
        this.indexWriteBlockedMetric = metricFactory.metricClusterIndexWritesBlocked();
        this.nodesMetric = metricFactory.metricNodes();
        this.automatedSnapshotFailureMetric = metricFactory.metricAutomatedSnapshotFailure();
        ...
        for (const disambiguator in props.addMasterCpuSpaceUsageAlarm) {
            ...
            const createdAlarm = this.usageAlarmFactory.addMaxMasterCpuUsagePercentAlarm(
        ...
        for (const disambiguator in props.addMasterJvmMemoryPressureAlarm) {
            ...
            const createdAlarm = this.usageAlarmFactory.addMaxMasterMemoryUsagePercentAlarm(
        ...
        for (const disambiguator in props.addClusterIndexWritesBlockedAlarm) {
            ...
            const createdAlarm = this.clusterAlarmFactory.addClusterIndexWritesBlockedAlarm(
        ...
        for (const disambiguator in props.addClusterAutomatedSnapshotFailureAlarm) {
            ...
            const createdAlarm = this.clusterAlarmFactory.addAutomatedSnapshotFailureAlarm(
    ...
    summaryWidgets(): IWidget[] {
        ...
        return [
            ...
            new GraphWidget({
                ...
                left: [this.p50IndexingLatencyMetric, this.p90IndexingLatencyMetric, this.p99IndexingLatencyMetric],
            ...
            new GraphWidget({
                ...
                left: [this.p50SearchLatencyMetric, this.p90SearchLatencyMetric, this.p99SearchLatencyMetric],
                leftYAxis: TimeAxisMillisFromZero,
            ...
            new GraphWidget({
                ...
                leftAnnotations: [...this.usageAnnotations, ...this.masterUsageAnnotations],
            }),
        ];
    }
        ...
        return [
            ...
            new Row(
                ...
                new GraphWidget({
                    ...
                    height: DefaultGraphWidgetHeight,
                    title: "CPU/Memory/Disk Usage",
                    left: [this.cpuUsageMetric, this.jvmMemoryPressureMetric, this.diskSpaceUsageMetric],
                    leftYAxis: PercentageAxisFromZeroToHundred,
