...
export class RedshiftClusterMetricFactory {
    ...
    metricTotalConnectionCount() {
        return this.metric("DatabaseConnections", MetricStatistic.SUM, "Connections");
    }

    metricAverageDiskSpaceUsageInPercent() {
        return this.metric("PercentageDiskSpaceUsed", MetricStatistic.AVERAGE, "Disk Usage");
    }
    ...
    metricShortQueryDurationP90InMillis() {
        const sQueryLatency90 = this.metricQueryDuration("short", MetricStatistic.P90);
    ...
    metricMediumQueryDurationP90InMillis() {
        const mQueryLatency90 = this.metricQueryDuration("medium", MetricStatistic.P90);
        // need to convert micros to millis
    ...
    metricLongQueryDurationP90InMillis() {
        const lQueryLatency90 = this.metricQueryDuration("long", MetricStatistic.P90);
        // need to convert micros to millis
    ...
    metricReadLatencyP90InMillis() {
        const readLatency = this.metric("ReadLatency", MetricStatistic.P90, "Read P90");
    ...
    metricWriteLatencyP90InMillis() {
        const writeLatency = this.metric("WriteLatency", MetricStatistic.P90, "Write P90");
        // need to convert seconds to millis
        return this.metricFactory.createMetricMath("writeLatency * 1000", {writeLatency}, "Write P90");
    }

    private metricQueryDuration(latency: QueryLatencyClass, statistic: MetricStatistic) {
