...
import {RedshiftClusterMetricFactory, RedshiftClusterMetricFactoryProps} from "./RedshiftClusterMetricFactory";
import {
    BaseMonitoringProps,
...
export class RedshiftClusterMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: RedshiftClusterMonitoringProps) {
        ...
        this.url = scope.createAwsConsoleUrlFactory().getRedshiftClusterUrl(props.clusterIdentifier);
        ...
        const metricFactory = new RedshiftClusterMetricFactory(scope.createMetricFactory(), props);
        this.connectionsMetric = metricFactory.metricTotalConnectionCount();
        this.diskSpaceUsageMetric = metricFactory.metricAverageDiskSpaceUsageInPercent();
        this.cpuUsageMetric = metricFactory.metricAverageCpuUsageInPercent();
        this.shortQueryDurationMetric = metricFactory.metricShortQueryDurationP90InMillis();
        this.mediumQueryDurationMetric = metricFactory.metricMediumQueryDurationP90InMillis();
        this.longQueryDurationMetric = metricFactory.metricLongQueryDurationP90InMillis();
    ...
    createQueryDurationWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            left: [this.shortQueryDurationMetric, this.mediumQueryDurationMetric, this.longQueryDurationMetric],
