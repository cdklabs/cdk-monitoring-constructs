...
export class FargateServiceMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: CustomFargateServiceMonitoringProps) {
        ...
        this.hasLoadBalancer = props.loadBalancer !== undefined && props.targetGroup !== undefined;
        ...
        this.baseServiceMetricFactory = new BaseServiceMetricFactory(this.metricFactory, {service: props.fargateService});
        if (this.hasLoadBalancer) {
            this.loadBalancerMetricFactory = createLoadBalancerMetricFactory(this.metricFactory, props.loadBalancer!, props.targetGroup!);
            this.healthyTaskCountMetric = this.loadBalancerMetricFactory.metricHealthyTaskCount();
            this.unhealthyTaskCountMetric = this.loadBalancerMetricFactory.metricUnhealthyTaskCount();
            this.healthyTaskPercentMetric = this.loadBalancerMetricFactory.metricHealthyTaskInPercent();
            this.activeTcpFlowCountMetric = this.loadBalancerMetricFactory.metricActiveConnectionCount();
            this.newTcpFlowCountMetric = this.loadBalancerMetricFactory.metricNewConnectionCount();
            this.processedBytesMetric = this.loadBalancerMetricFactory.metricProcessedBytesMin();
        }
        this.runningTaskCountMetric = this.baseServiceMetricFactory.metricRunningTaskCount();
        this.cpuUtilisationMetric = this.baseServiceMetricFactory.metricClusterCpuUtilisationInPercent();
        this.memoryUtilisationMetric = this.baseServiceMetricFactory.metricClusterMemoryUtilisationInPercent();
    ...
    widgets(): IWidget[] {
        const baseWidget = [
            this.createTitleWidget(),
            this.createCpuWidget(this.hasLoadBalancer ? QuarterWidth : ThirdWidth, DefaultGraphWidgetHeight),
            this.createMemoryWidget(this.hasLoadBalancer ? QuarterWidth : ThirdWidth, DefaultGraphWidgetHeight),
        ];
        ...
        if (this.hasLoadBalancer) {
            ...
            return baseWidget.concat(this.createTaskHealthWidget(ThirdWidth, DefaultGraphWidgetHeight));
        }
    }
