...
import {GlueJobMetricFactory, GlueJobMetricFactoryProps} from "./GlueJobMetricFactory";
import {
    AlarmFactory,
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class GlueJobMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: GlueJobMonitoringProps) {
        ...
        const metricFactory = new GlueJobMetricFactory(scope.createMetricFactory(), props);
        ...
        this.cpuUsageMetric = metricFactory.metricAverageExecutorCpuUsagePercentage().with({label: "CPU"});
        this.heapMemoryUsageMetric = metricFactory.metricAverageExecutorMemoryUsagePercentage().with({label: "Heap"});
        ...
        this.failedTaskCountMetric = metricFactory.metricFailedTasksSum().with({label: "Failed"});
        this.failedTaskRateMetric = metricFactory.metricFailedTasksRate();
        this.killedTaskCountMetric = metricFactory.metricKilledTasksSum().with({label: "Killed"});
    ...
    createJobExecutionWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            height,
            title: "Job Execution",
            left: [this.activeExecutorsMetric, this.neededExecutorsMetric, this.completedStagesMetric],
