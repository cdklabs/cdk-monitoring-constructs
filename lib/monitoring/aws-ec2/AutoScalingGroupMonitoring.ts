...
import {AutoScalingGroupMetricFactory, AutoScalingGroupMetricFactoryProps} from "./AutoScalingGroupMetricFactory";
import {
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class AutoScalingGroupMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: AutoScalingGroupMonitoringProps) {
        ...
        const metricFactory = new AutoScalingGroupMetricFactory(scope.createMetricFactory(), props);
        ...
        this.instancesInServiceMetric = metricFactory.metricGroupInServiceInstances();
        ...
        this.instancesTerminatingMetric = metricFactory.metricGroupTerminatingInstances();
        this.instancesTotalMetric = metricFactory.metricGroupTotalInstances();
    }

    summaryWidgets(): IWidget[] {
        return [this.createTitleWidget(), this.createGroupSizeWidget(FullWidth, DefaultSummaryWidgetHeight)];
    }
    ...
    createGroupSizeWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            height,
            title: "Group Size",
            left: [this.groupMinSizeMetric, this.groupMaxSizeMetric, this.groupDesiredSizeMetric, this.instancesTotalMetric],
