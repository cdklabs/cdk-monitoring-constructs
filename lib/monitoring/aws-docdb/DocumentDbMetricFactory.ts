...
import {getLatencyTypeLabel, getLatencyTypeStatistic, LatencyType, MetricFactory, MetricStatistic} from "../../common";
...
export class DocumentDbMetricFactory {
    ...
    metricMaxConnectionCount() {
        return this.metric("DatabaseConnectionsMax", MetricStatistic.MAX, "Connections");
    }
    ...
    metricMaxTransactionOpenCount() {
        return this.metric("TransactionsOpenMax", MetricStatistic.MAX, "Transactions");
    }

    metricOperationsThrottledDueLowMemoryCount() {
        return this.metric("LowMemNumOperationsThrottled", MetricStatistic.SUM, "Operations throttled (low mem)");
    }

    metricReadLatencyInMillis(latencyType: LatencyType) {
        const label = "Read " + getLatencyTypeLabel(latencyType);
        return this.metric("ReadLatency", getLatencyTypeStatistic(latencyType), label);
    }

    metricWriteLatencyInMillis(latencyType: LatencyType) {
        const label = "Write " + getLatencyTypeLabel(latencyType);
        return this.metric("WriteLatency", getLatencyTypeStatistic(latencyType), label);
    }
