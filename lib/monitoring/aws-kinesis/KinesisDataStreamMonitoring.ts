...
import {KinesisDataStreamMetricFactory, KinesisDataStreamMetricFactoryProps} from "./KinesisDataStreamMetricFactory";
import {
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class KinesisDataStreamMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: KinesisDataStreamMonitoringProps) {
        ...
        this.streamUrl = scope.createAwsConsoleUrlFactory().getKinesisDataStreamUrl(props.streamName);
        ...
        const metricFactory = new KinesisDataStreamMetricFactory(scope.createMetricFactory(), props);
        this.metricGetRecordSumBytes = metricFactory.metricGetRecordsSumBytes();
        this.metricGetRecordsIteratorAge = metricFactory.metricGetRecordsIteratorAgeMaxMs();
        this.metricGetRecordsLatencyAverage = metricFactory.metricGetRecordsLatencyAverageMs();
        this.metricGetRecordsSumCount = metricFactory.metricGetRecordsSumCount();
        this.metricGetRecordsSuccessCount = metricFactory.metricGetRecordsSuccessCount();
        this.incomingDataSumBytesMetric = metricFactory.metricIncomingDataSumBytes();
        this.incomingDataSumCountMetric = metricFactory.metricIncomingDataSumCount();
        this.putRecordSumBytesMetric = metricFactory.metricPutRecordSumBytes();
        this.putRecordLatencyAverageMetric = metricFactory.metricPutRecordLatencyAverageMs();
        this.putRecordSuccessCountMetric = metricFactory.metricPutRecordSuccessCount();
        this.putRecordsSumBytesMetric = metricFactory.metricPutRecordsSumBytes();
        this.putRecordsLatencyAverageMetric = metricFactory.metricPutRecordsLatencyAverageMs();
        this.putRecordsSuccessCountMetric = metricFactory.metricPutRecordsSuccessCount();
        this.putRecordsTotalRecordsCountMetric = metricFactory.metricPutRecordsTotalRecordsCount();
        this.putRecordsSuccessfulRecordsCountMetric = metricFactory.metricPutRecordsSuccessfulRecordsCount();
        this.putRecordsThrottledRecordsCountMetric = metricFactory.metricPutRecordsThrottledRecordsCount();
        this.putRecordsFailedRecordsCountMetric = metricFactory.metricPutRecordsFailedRecordsCount();
        this.readProvisionedThroughputExceededMetric = metricFactory.metricReadProvisionedThroughputExceededPercent();
        this.writeProvisionedThroughputExceededMetric = metricFactory.metricWriteProvisionedThroughputExceededPercent();
        ...
        for (const disambiguator in props.addReadProvisionedThroughputExceededAlarm) {
            ...
            const createdAlarm = this.kinesisAlarmFactory.addProvisionedReadThroughputExceededAlarm(
        ...
        for (const disambiguator in props.addWriteProvisionedThroughputExceededAlarm) {
            ...
            const createdAlarm = this.kinesisAlarmFactory.addProvisionedWriteThroughputExceededAlarm(
    ...
    createSecondAdditionalRow() {
        return new Row(this.createRecordNumberWidget(FullWidth, DefaultGraphWidgetHeight));
    }
    ...
    createLatencyWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            left: [this.metricGetRecordsLatencyAverage, this.putRecordLatencyAverageMetric, this.putRecordsLatencyAverageMetric],
            leftYAxis: TimeAxisMillisFromZero,
        });
    }
    ...
    createCapacityWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            left: [this.readProvisionedThroughputExceededMetric, this.writeProvisionedThroughputExceededMetric],
            leftYAxis: TimeAxisMillisFromZero,
    ...
    createRecordSizeWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            height,
            title: "Bytes",
            left: [this.metricGetRecordSumBytes, this.putRecordSumBytesMetric, this.putRecordsSumBytesMetric],
    ...
    createOperationWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            left: [this.metricGetRecordsSuccessCount, this.putRecordSuccessCountMetric, this.putRecordsSuccessCountMetric],
            leftYAxis: CountAxisFromZero,
        });
    }
