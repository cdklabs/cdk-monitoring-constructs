...
import {SqsQueueMetricFactory, SqsQueueMetricFactoryProps} from "./SqsQueueMetricFactory";
import {
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class SqsQueueMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: SqsQueueMonitoringProps, invokedFromSuper?: boolean) {
        ...
        this.queueUrl = scope.createAwsConsoleUrlFactory().getSqsQueueUrl(props.queue.queueUrl);
        ...
        const metricFactory = new SqsQueueMetricFactory(scope.createMetricFactory(), props);
        this.visibleMessagesMetric = metricFactory.metricApproximateVisibleMessageCount();
        ...
        this.oldestMessageAgeMetric = metricFactory.metricApproximateAgeOfOldestMessageInSeconds();
        ...
        for (const disambiguator in props.addQueueMaxTimeToDrainMessagesAlarm) {
            ...
            const createdAlarm = this.queueAlarmFactory.addMaxQueueTimeToDrainMessagesAlarm(
        ...
        for (const disambiguator in props.addQueueMinIncomingMessagesAlarm) {
            ...
            const createdAlarm = this.queueAlarmFactory.addMinQueueIncomingMessagesCountAlarm(
        ...
        for (const disambiguator in props.addQueueMaxIncomingMessagesAlarm) {
            ...
            const createdAlarm = this.queueAlarmFactory.addMaxQueueIncomingMessagesCountAlarm(
                this.incomingMessagesMetric,
                alarmProps,
                disambiguator,
            );
    ...
    summaryWidgets(): IWidget[] {
        return [
            ...
            new Row(
                this.createProducerAndConsumerRateWidget(HalfWidth, DefaultSummaryWidgetHeight),
    ...
    widgets(): IWidget[] {
        return [
            ...
            new Row(
                this.createProducerAndConsumerRateWidget(HalfWidth, DefaultGraphWidgetHeight),
