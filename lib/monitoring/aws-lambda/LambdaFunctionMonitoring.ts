...
export class LambdaFunctionMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: LambdaFunctionMonitoringProps) {
        ...
        this.functionUrl = scope.createAwsConsoleUrlFactory().getLambdaFunctionUrl(props.lambdaFunction.functionName);
        ...
        this.metricFactory = new LambdaFunctionMetricFactory(scope.createMetricFactory(), props);
        ...
        this.concurrentExecutionsCountMetric = this.metricFactory.metricConcurrentExecutions();
        this.provisionedConcurrencySpilloverInvocationsCountMetric = this.metricFactory.metricProvisionedConcurrencySpilloverInvocations();
        this.provisionedConcurrencySpilloverInvocationsRateMetric = this.metricFactory.metricProvisionedConcurrencySpilloverRate();
        this.maxIteratorAgeMetric = this.metricFactory.metricMaxIteratorAgeInMillis();

        this.lambdaInsightsEnabled = props.lambdaInsightsEnabled ?? false;
        if (props.lambdaInsightsEnabled) {
            this.enhancedMetricFactory = new LambdaFunctionEnhancedMetricFactory(scope.createMetricFactory(), props.lambdaFunction);
            this.enhancedMonitoringMaxCpuTotalTimeMetric = this.enhancedMetricFactory.enhancedMetricMaxCpuTotalTime();
            this.enhancedMonitoringP90CpuTotalTimeMetric = this.enhancedMetricFactory.enhancedMetricP90CpuTotalTime();
            this.enhancedMonitoringAvgCpuTotalTimeMetric = this.enhancedMetricFactory.enhancedMetricAvgCpuTotalTime();
            this.enhancedMonitoringMaxMemoryUtilizationMetric = this.enhancedMetricFactory.enhancedMetricMaxMemoryUtilization();
            this.enhancedMonitoringP90MemoryUtilizationMetric = this.enhancedMetricFactory.enhancedMetricP90MemoryUtilization();
            this.enhancedMonitoringAvgMemoryUtilizationMetric = this.enhancedMetricFactory.enhancedMetricAvgMemoryUtilization();
            this.enhancedMetricFunctionCostMetric = this.enhancedMetricFactory.enhancedMetricFunctionCost();
    ...
    widgets(): IWidget[] {
        ...
        if (this.lambdaInsightsEnabled) {
            widgets.push(
                new Row(
                    this.createLambdaInsightsCpuWidget(ThirdWidth, DefaultGraphWidgetHeight),
                    this.createLambdaInsightsMemoryWidget(ThirdWidth, DefaultGraphWidgetHeight),
                    this.createLambdaInsightsFunctionCostWidget(ThirdWidth, DefaultGraphWidgetHeight),
                ),
            );
        }
    ...
    createLatencyWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            height,
            title: "Latency",
            left: [this.p50LatencyMetric, this.p90LatencyMetric, this.p99LatencyMetric],
    ...
    createRateWidget(width: number, height: number) {
        return new GraphWidget({
            ...
            height,
            title: "Rates",
            left: [this.throttlesRateMetric, this.provisionedConcurrencySpilloverInvocationsRateMetric],
            leftYAxis: RateAxisFromZero,
