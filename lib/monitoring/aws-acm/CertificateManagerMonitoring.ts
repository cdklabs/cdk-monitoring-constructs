...
import {CertificateManagerMetricFactory, CertificateManagerMetricFactoryProps} from "./CertificateManagerMetricFactory";
import {
    AgeAlarmFactory,
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class CertificateManagerMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: CertificateManagerMonitoringProps) {
        ...
        const metricFactory = new CertificateManagerMetricFactory(scope.createMetricFactory(), props);
    ...
    widgets(): IWidget[] {
        return [this.createTitleWidget(), this.createDaysToExpiryWidget(QuarterWidth, DefaultGraphWidgetHeight)];
    }
