...
import {S3BucketMetricFactory, S3BucketMetricFactoryProps} from "./S3BucketMetricFactory";
import {
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class S3BucketMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: S3BucketMonitoringProps) {
        ...
        this.url = scope.createAwsConsoleUrlFactory().getS3BucketUrl(props.bucket.bucketName);

        const metricFactory = new S3BucketMetricFactory(this.createMetricFactory(), props);
