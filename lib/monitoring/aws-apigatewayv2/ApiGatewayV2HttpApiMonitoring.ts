...
import {ApiGatewayV2HttpApiMetricFactory, ApiGatewayV2HttpApiMetricFactoryProps} from "./ApiGatewayV2HttpApiMetricFactory";
import {
    AlarmFactory,
    BaseMonitoringProps,
    CountAxisFromZero,
...
const DefaultLatencyTypesToRender = [LatencyType.P50, LatencyType.P90, LatencyType.P99];
...
export class ApiGatewayV2HttpApiMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: ApiGatewayV2HttpApiMonitoringProps) {
        ...
        const namingStrategy = new MonitoringNamingStrategy({
            ...
            fallbackConstructName: fallbackNameArray.join("-").replace(/[^a-zA-Z0-9-_]/g, ""),
        ...
        const metricFactory = new ApiGatewayV2HttpApiMetricFactory(scope.createMetricFactory(), props);

        this.tpsMetric = metricFactory.metricTps();

        this.latencyMetrics = {};
        this.integrationLatencyMetrics = {};
        this.latencyTypesToRender = [...(props.latencyTypesToRender ?? DefaultLatencyTypesToRender)];
        ...
        Object.values(LatencyType).forEach((latencyType) => {
            this.latencyMetrics[latencyType] = metricFactory.metricLatencyInMillis(latencyType);
            this.integrationLatencyMetrics[latencyType] = metricFactory.metricIntegrationLatencyInMillis(latencyType);
        });
        ...
        for (const [latencyType, alarmDefinition] of Object.entries(integrationLatencyAlarmDefinitions)) {
            for (const disambiguator in alarmDefinition) {
                ...
                const createdAlarm = this.latencyAlarmFactory.addIntegrationLatencyAlarm(
                    metric,
                    latencyTypeEnum,
                    alarmProps,
                    disambiguator,
