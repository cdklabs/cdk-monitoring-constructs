...
import {StepFunctionMetricFactory, StepFunctionMetricFactoryProps} from "./StepFunctionMetricFactory";
import {
    BaseMonitoringProps,
    CountAxisFromZero,
...
export class StepFunctionMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: StepFunctionMonitoringProps) {
        ...
        this.stateMachineUrl = scope.createAwsConsoleUrlFactory().getStateMachineUrl(stateMachineArn);
        ...
        const metricFactory = new StepFunctionMetricFactory(scope.createMetricFactory(), props);
        ...
        for (const disambiguator in props.addMinStartedExecutionCountAlarm) {
            ...
            const createdAlarm = this.taskHealthAlarmFactory.addMinRunningTaskCountAlarm(
