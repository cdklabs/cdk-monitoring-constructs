import {GraphWidget, GraphWidgetView, IMetric, IWidget, SingleValueWidget} from "aws-cdk-lib/aws-cloudwatch";

import {BillingCurrency, BillingMetricFactory, BillingRegion} from "./BillingMetricFactory";
import {
    AlarmFactory,
...
export class BillingMonitoring extends Monitoring {
    ...
    constructor(scope: MonitoringScope, props: BillingMonitoringProps) {
        ...
        this.anomalyDetectingAlarmFactory = new AnomalyDetectingAlarmFactory(this.alarmFactory);
        const metricFactory = new BillingMetricFactory();
        this.costByServiceMetric = metricFactory.metricSearchTopCostByServiceInUsd();
    ...
    summaryWidgets(): IWidget[] {
        return [this.createTitleWidget(), this.createTotalChargesWidget(FullWidth, DefaultSummaryWidgetHeight)];
    }

    widgets(): IWidget[] {
        return [
            this.createTitleWidget(),
            this.createChargesByServiceWidget(ThreeQuartersWidth, DefaultGraphWidgetHeight),
