...
export function notifySns(onAlarmTopic: ITopic, onOkTopic?: ITopic, onInsufficientDataTopic?: ITopic): IAlarmActionStrategy {
...
export class SnsAlarmActionStrategy implements IAlarmActionStrategy {
    ...
    addAlarmActions(props: AlarmActionStrategyProps): void {
        ...
        if (this.onInsufficientDataTopic) {
            props.alarm.addInsufficientDataAction(new SnsAction(this.onInsufficientDataTopic));
        }
    }
}
