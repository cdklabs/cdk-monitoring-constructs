...
import {AlarmAnnotationStrategyProps, DefaultAlarmAnnotationStrategy, IAlarmAnnotationStrategy} from "./IAlarmAnnotationStrategy";
import {IAlarmDedupeStringProcessor} from "./IAlarmDedupeStringProcessor";
import {IAlarmNamingStrategy} from "./IAlarmNamingStrategy";
import {MetricFactoryDefaults, MetricStatistic, MetricWithAlarmSupport} from "../metric";
 ...
 */
export type ActionsEnabled = boolean | Record<PredefinedAlarmDisambiguators | string, boolean>;
...
export class AlarmFactory {
    ...
    addAlarm(metric: MetricWithAlarmSupport, props: AddAlarmProps): AlarmWithAnnotation {
        ...
        const datapointsToAlarm = props.datapointsToAlarm ?? this.globalAlarmDefaults.datapointsToAlarm ?? DefaultDatapointsToAlarm;
        const evaluationPeriods = props.evaluationPeriods ?? this.globalAlarmDefaults.evaluationPeriods ?? datapointsToAlarm;
        ...
        const primaryAlarm = adjustedMetric.createAlarm(this.alarmScope, alarmName, {
            ...
            evaluateLowSampleCountPercentile: evaluateLowSampleCountPercentile ? undefined : "ignore",
            datapointsToAlarm,
            evaluationPeriods,
            actionsEnabled,
        });
        ...
        return {
            ...
            alarmRuleWhenInsufficientData: AlarmRule.fromAlarm(alarm, AlarmState.INSUFFICIENT_DATA),
            dedupeString,
            annotation,
            disambiguator: props.disambiguator,
        };
    ...
    addCompositeAlarm(alarms: AlarmWithAnnotation[], props: AddCompositeAlarmProps): CompositeAlarm {
        const actionsEnabled = this.determineActionsEnabled(props?.actionsEnabled, props?.disambiguator);
        const action = props.actionOverride ?? this.globalAlarmDefaults.action ?? noopAction();
    ...
    protected determineAction(disambiguator?: string, actionOverride?: IAlarmActionStrategy): IAlarmActionStrategy {
        ...
        if (disambiguator && this.globalAlarmDefaults.disambiguatorAction?.[disambiguator]) {
    ...
    protected createAnnotation(props: AlarmAnnotationStrategyProps) {
        const strategy = this.globalAlarmDefaults.annotationStrategy ?? new DefaultAlarmAnnotationStrategy();
