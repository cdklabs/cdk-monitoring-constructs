...
import {DimensionsMap, IMetric, MathExpression, Metric} from "aws-cdk-lib/aws-cloudwatch";
...
export class MetricFactory {
    ...
    createMetric(
        ...
        return new Metric({
            ...
            label,
            color,
            dimensionsMap: dimensionsMap ? this.removeUndefinedEntries(dimensionsMap) : undefined,
    ...
    createMetricSearch(
        ...
        const finalPeriod = period ?? this.globalDefaults.period ?? DefaultMetricPeriod;
        const searchNamespace = this.getNamespaceWithFallback(namespace);
        const namespacePlusDimensionKeys = [searchNamespace, ...Object.keys(dimensionsMap)].join(",");
    ...
    toRate(
        ...
        const finalExpressionIdZeroed = fillWithZeroes ?? false ? `FILL(${finalExpressionId},0)` : finalExpressionId;
        ...
        switch (method) {
            ...
            case RateComputationMethod.PER_SECOND:
                ...
                if (labelPrefix === "Requests" || labelPrefix === "Invocations" || labelPrefix === "Transactions") {
     ...
     */
    addAdditionalDimensions(target: DimensionsMap, additionalDimensions: DimensionsMap) {
